...
class PrintInput:
    ...
    def read_file(self, inputfile):
        ...
        with open(inputfile) as f:
            lines = f.readlines()
            template = Template("".join(lines))  # or just f.read() if needed as raw text
        ...
        return atoms, template.render()


    #def read_file(self,inputfile):
    #    with open(inputfile) as f:
    #        template = Template(f.read())
    #        for line in f[2:]:  # Skip the first two lines (atom count and comment)
    #            parts = line.split()
    #            atom = parts[0]
    #    return atoms, template
    ...
    def do_saoovqe(self, inputfile):
        ...
        atomid, file = self.read_file(inputfile) 
        print(file)
        saoovqe_class = SAOOVQE(file,
                  basis,
                  nelec_active,
                  frozen_indices,
                  active_indices,
        ...
        for i in range(natoms):
            ...
            nac.append([-nx,-ny,-nz])
        output = inputfile.replace('.input', '.log')
        with open(output, 'w') as f:
            f.write(f"SA-OO-VQE Quantum Algorithm\n")
            f.write(f"\n[Energies]\n")
            f.write(f"Energy_0: {self.energies[0]}\n")
            f.write(f"Energy_1: {self.energies[1]}\n")
            f.write(f"[Nonadiabatic_Couplings]\n")
            nacs = self._nacs(atomid, nac)
            for key, val in nacs.items():
                f.write(f"{key}:\n")
                for row in val:
                    f.write("  " + "  ".join(map(str, row)) + "\n")

    def _nacs(self, atomid, nac):
        nacs = {}
        # Combine atom label and NAC values
        combined = [[s] + n for s, n in zip(atomid, nac)]
        arr = np.array(combined, dtype=object)
        # Create a negated version: negate only the numeric part
        neg_arr = np.array([[row[0]] + [-x for x in row[1:]] for row in arr], dtype=object)
