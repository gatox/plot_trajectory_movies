...
class PlotResults:
    ...
    def plot_pos_vel(self):
        fig, ax = plt.subplots()
        crd = np.array(self.dis_two_atmos("crd", 0, 1))
        vel = np.array(self.dis_two_atmos("veloc", 0, 1))
    ...
    def plot_time_total_energy(self):
        etot = np.array(self.output["etot"])
        plt.plot(self.time,(etot-etot[0])/1e-3,color='blue',label = '') 
    ...
    def plot_time_parmameter(self):
        parameter = np.array(self.output["parameter"])
        plt.plot(self.time,parameter,color='blue',label = '') 
        plt.xlabel('Time (fs)', fontweight = 'bold', fontsize = 16)
...
if __name__=="__main__":
    ...
    picture.plot_pos_vel()
    picture.plot_time_total_energy()
    picture.plot_time_parmameter()
