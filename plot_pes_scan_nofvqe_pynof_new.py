...
def safe_merge(backup_val, primary_val):
    # If primary is completely missing, use backup
    if primary_val is None:
        return backup_val
    
    # If primary is a string representation of nan, use backup
    if isinstance(primary_val, str):
        if primary_val.strip().lower() in ['nan', 'none', 'null', '']:
            return backup_val
        try:
    ...
    # If primary is a mathematical float nan, use backup
    try:
        if math.isnan(float(primary_val)):
            return backup_val
    except (TypeError, ValueError):
        return backup_val
        
    # Otherwise, primary is perfectly valid!
    return primary_val
...
def compute_pes(base_dir="cube_h2_pynof", save_prefix="pynof_pes", guess=False, device=None, hf_only=False):
    ...
    for folder in sorted(base_path.iterdir()):
        ...
        # Convert the Path objects to strings first, then check for "eva"
        file_0_str = str(out_files[0]).lower()

        # 1. Identify which file is 'eva' and which is 'cube'
        if "eva" in file_0_str:
            eva_file  = out_files[0]
            cube_file = out_files[1]
        else:
            eva_file  = out_files[1]
            cube_file = out_files[0]

        # 2. Extract data from both files independently
        hf_eva, nof_eva, corr_eva, sim_nof_eva, sim_corr_eva = extract_energies_from_out(eva_file, device)
        hf_cube, nof_cube, corr_cube, sim_nof_cube, sim_corr_cube = extract_energies_from_out(cube_file, device)

        # 3. Complement columns: Try to use 'eva' data. If 'eva' is nan, use 'cube' data instead.
        hf       = safe_merge(hf_cube, hf_eva)
        nof      = safe_merge(nof_cube, nof_eva)
        corr     = safe_merge(corr_cube, corr_eva)
        sim_nof  = safe_merge(sim_nof_cube, sim_nof_eva)
        sim_corr = safe_merge(sim_corr_cube, sim_corr_eva)

        #out_file = out_files[1]
        #hf, nof, corr, sim_nof, sim_corr = extract_energies_from_out(out_file, device)

        if hf is None or corr is None:
            print(f"[WARNING] Could not extract HF energy")
            #continue

        distances.append(d_ang)
        hf_energies.append(hf if hf is not None else np.nan)
...
def plot_pes_sim_qc_stats(
    ...
    # -------------------------------------------------------------
    # NEW: Clean out the 3.75 coordinate completely from the dataset
    # -------------------------------------------------------------
    clean_mask = ~np.isclose(arr_ref[:, 0], 3.75)
    arr_ref = arr_ref[clean_mask]
    # -------------------------------------------------------------
    ...
    for irun, data_qc in enumerate(data_qc_list):
        ...
        # -------------------------------------------------------------
        # NEW: Apply the same 3.75 filter to each loop file
        # -------------------------------------------------------------
        arr_qc = arr_qc[~np.isclose(arr_qc[:, 0], 3.75)]
        # -------------------------------------------------------------
...
if __name__ == "__main__":                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
    ...
    #     )                        
        ...
        plot_pes_filtered(
            records,
            save_prefix=str(base_path / "pes_H64_real_filtered"),
        )

        plot_acceptance_rate(
            records,
            save_prefix=str(base_path / "pes_H64_real_acceptance"),
        )     
    elif option == "plot":
        ...
        plot_pes_sim_qc_stats(
            qc_data,
            save_prefix=str(base_path / "pes_H64_scan_hybrid_real_mean_std"),
        )                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
